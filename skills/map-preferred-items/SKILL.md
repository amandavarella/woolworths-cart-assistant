...
3. Loads your ignore list from `ignore-items.txt` (one item per line; `#` comments ignored). Any ingredient matching an entry (case-insensitive, word-based, so "Cassava" also drops "cassava flour") is **dropped** — never mapped, never added to the cart. Ignored items are recorded in the plan under `ignored`.
...
```json
{
  ...
  "ignoredCount": 1,
  "ignored": [
    { "source": "anylist", "name": "Cassava", "ingredient": "Cassava" }
  ],
  "plan": [
    {
      "source": "clove",
...
- Edit `ignore-items.txt` to drop items you never want bought (e.g. things you grow or get elsewhere). One item per line; matching is case-insensitive and word-based.
